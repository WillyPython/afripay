...
def render_sidebar(user: dict | None) -> None:
    ...
    selected_plan_choice = clean_text(
        st.session_state.get("selected_plan_choice", "")
    ).upper()
    if selected_plan_choice not in {PLAN_FREE, PLAN_PREMIUM, PLAN_PREMIUM_PLUS}:
        selected_plan_choice = ""

    selected_plan_duration = int(st.session_state.get("selected_plan_duration", 6) or 6)
    if selected_plan_duration not in PLAN_DURATION_OPTIONS:
        selected_plan_duration = 6

    # ---------------------------------
    # Carte sidebar cohérente avec le choix en cours
    # ---------------------------------
    if selected_plan_choice == PLAN_PREMIUM:
        sidebar_card_title = "Option choisie" if get_language() == "fr" else "Selected option"
        sidebar_card_value = tr("plan_premium")
        sidebar_card_subline = (
            "Activation en attente de confirmation."
            if get_language() == "fr"
            else "Activation pending confirmation."
        )
    elif selected_plan_choice == PLAN_PREMIUM_PLUS:
        sidebar_card_title = "Option choisie" if get_language() == "fr" else "Selected option"
        sidebar_card_value = tr("plan_premium_plus")
        duration_label = (
            f"{selected_plan_duration} mois"
            if get_language() == "fr"
            else f"{selected_plan_duration} months"
        )
        sidebar_card_subline = duration_label
    else:
        sidebar_card_title = tr("sidebar_plan")
        sidebar_card_value = plan_label
        sidebar_card_subline = (
            f"{tr('sidebar_free_left')}: {free_left if effective_plan == PLAN_FREE else '∞'}"
        )

    st.sidebar.markdown(
        f"""
        <div class="af-card">
            <div style="font-size:0.9rem;color:#475569;">{sidebar_card_title}</div>
            <div style="font-size:1.1rem;font-weight:700;">{sidebar_card_value}</div>
            <div class="af-small">{sidebar_card_subline}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if selected_plan_choice == PLAN_PREMIUM:
        ...
        st.sidebar.success(
            f"Option PREMIUM_PLUS sélectionnée ({duration_label})."
            if get_language() == "fr"
            else f"PREMIUM_PLUS option selected ({duration_label})."
        )
        ...
        if should_show_upgrade_button:
            if effective_plan == PLAN_FREE and user and free_left <= 0:
                st.sidebar.success(
                    "Upgrade activé : passez à PREMIUM ou PREMIUM_PLUS."
                    if get_language() == "fr"
                    else "Upgrade enabled: switch to PREMIUM or PREMIUM_PLUS."
                )
            else:
                if upgrade_available:
                    ...
                    st.sidebar.caption(tr("sidebar_upgrade_unavailable"))
    ...
    if user:
        if st.sidebar.button(tr("sidebar_logout"), width=UI_WIDTH_STRETCH):
            ...
            for key in [
                ...
                "last_order_code",
                "selected_plan_choice",
                "selected_plan_duration",
                "premium_plus_subscription_payment_method_selected",
                "premium_plus_confirmation_checkbox",
            ]:
                st.session_state.pop(key, None)
...
def render_plan_cards(user: dict | None) -> None:
    ...
    with col1:
        ...
        if st.button(
            "Créer une commande FREE" if get_language() == "fr" else "Create FREE order",
            key="plan_free_create_order",
            width=UI_WIDTH_STRETCH,
        ):
            st.session_state["selected_plan_choice"] = PLAN_FREE
    ...
    with col2:
        ...
        if st.button(
            "Créer une commande PREMIUM" if get_language() == "fr" else "Create PREMIUM order",
            key="plan_premium_create_order",
            width=UI_WIDTH_STRETCH,
        ):
            st.session_state["selected_plan_choice"] = PLAN_PREMIUM
            st.session_state["premium_page_open"] = False
            st.rerun()
    ...
    with col3:
        ...
        duration = st.radio(
            ...
            index=0,
            horizontal=True,
            key="plan_card_premium_plus_duration",
        )

        price = get_premium_plus_price(duration)
        if price:
            st.caption(
                f"{duration} mois : {price}"
                if get_language() == "fr"
                else f"{duration} months: {price}"
            )

        if st.button(
            "Créer une commande PREMIUM_PLUS" if get_language() == "fr" else "Create PREMIUM_PLUS order",
            key="plan_premium_plus_create_order",
            width=UI_WIDTH_STRETCH,
        ):
            st.session_state["selected_plan_choice"] = PLAN_PREMIUM_PLUS
            st.session_state["selected_plan_duration"] = duration
            st.session_state["premium_page_open"] = False
            st.rerun()
...
def render_order_form(user: dict | None) -> None:
    ...
    selected_plan = clean_text(
        st.session_state.get("selected_plan_choice", get_effective_user_plan(user))
    ).upper()

    if selected_plan not in {PLAN_FREE, PLAN_PREMIUM, PLAN_PREMIUM_PLUS}:
        selected_plan = get_effective_user_plan(user)

    selected_duration = int(st.session_state.get("selected_plan_duration", 6) or 6)
    if selected_duration not in PLAN_DURATION_OPTIONS:
        selected_duration = 6

    if selected_plan == PLAN_FREE:
        selected_plan_label = tr("plan_free")
    elif selected_plan == PLAN_PREMIUM:
        selected_plan_label = tr("plan_premium")
    else:
        selected_plan_label = tr("plan_premium_plus")

    st.info(
        f"Option choisie : {selected_plan_label}"
        if get_language() == "fr"
        else f"Selected option: {selected_plan_label}"
    )

    premium_plus_confirmed = True
    premium_plus_subscription_payment_method = ""

    if selected_plan == PLAN_PREMIUM_PLUS:
        premium_plus_price = get_premium_plus_price(selected_duration)
        duration_label = (
            f"{selected_duration} mois"
            if get_language() == "fr"
            else f"{selected_duration} months"
        )
        ...
        st.markdown(
            ...
            <div class="af-warning-card">
                <h4>PREMIUM_PLUS</h4>
                <p>
                    {"Durée choisie" if get_language() == "fr" else "Selected duration"} : <strong>{duration_label}</strong><br>
                    {"Montant abonnement" if get_language() == "fr" else "Subscription amount"} :
                    <strong>{premium_plus_price or ('À confirmer' if get_language() == 'fr' else 'To be confirmed')}</strong><br><br>
                    {"L'accès PREMIUM_PLUS commence uniquement après paiement confirmé et validation administrateur. Tant que ce paiement n'est pas validé, l'avantage 0% n'est pas actif."
                     if get_language() == "fr"
                     else "PREMIUM_PLUS access starts only after confirmed payment and admin validation. Until that payment is not validated, the 0% fee benefit is not active."}
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        premium_plus_subscription_payment_method = st.radio(
            "Mode de paiement de l'abonnement PREMIUM_PLUS"
            if get_language() == "fr"
            else "PREMIUM_PLUS subscription payment method",
            options=["MTN MoMo", "Orange Money"],
            horizontal=True,
            key="premium_plus_subscription_payment_method",
        )

        premium_plus_confirmed = st.checkbox(
            "Je comprends que PREMIUM_PLUS devient actif seulement après paiement confirmé et validation admin."
            if get_language() == "fr"
            else "I understand that PREMIUM_PLUS becomes active only after confirmed payment and admin validation.",
            key="premium_plus_confirmation_checkbox",
        )

    user_country = get_user_country_code(user)
    upgrade_available = has_any_whatsapp_configured(user_country)

    simulated_user = dict(user or {})
    simulated_user["plan"] = selected_plan

    if selected_plan == PLAN_FREE and get_free_orders_remaining(simulated_user) <= 0:
        if upgrade_available:
            st.warning(tr("free_upgrade_ready"))
            st.session_state["premium_page_open"] = True
        else:
    ...
    if selected_plan == PLAN_PREMIUM_PLUS and not premium_plus_confirmed:
        st.error(
            "Vous devez confirmer la règle d'activation PREMIUM_PLUS avant de continuer."
            if get_language() == "fr"
            else "You must confirm the PREMIUM_PLUS activation rule before continuing."
        )
        return

    if selected_plan == PLAN_PREMIUM_PLUS and not clean_text(premium_plus_subscription_payment_method):
        st.error(
            "Veuillez sélectionner le mode de paiement de l'abonnement PREMIUM_PLUS."
            if get_language() == "fr"
            else "Please select the PREMIUM_PLUS subscription payment method."
        )
        return
    ...
    if selected_plan == PLAN_FREE:
        free_error = validate_free_order_rules(simulated_user, estimated_xaf)
    ...
    try:
        ...
        mark_premium_plus_payment_pending = None
        set_user_plan = None

    try:
        if selected_plan == PLAN_FREE and set_user_plan is not None:
            set_user_plan(int(user["id"]), PLAN_FREE)

        elif selected_plan == PLAN_PREMIUM and set_user_plan is not None:
            set_user_plan(int(user["id"]), PLAN_PREMIUM)

        elif selected_plan == PLAN_PREMIUM_PLUS:
            duration_code = "12M" if selected_duration == 12 else "6M"

            if mark_premium_plus_payment_pending is not None:
                mark_premium_plus_payment_pending(int(user["id"]), duration_code)
            elif set_user_plan is not None:
                set_user_plan(int(user["id"]), PLAN_PREMIUM_PLUS)

            st.session_state["premium_plus_subscription_payment_method_selected"] = clean_text(
                premium_plus_subscription_payment_method
            )
    ...
    try:
        created_result = create_order_for_user(
            ...
            momo_provider=clean_text(payment_method),
...
def render_order_success(user: dict | None, order: dict | None) -> None:
    ...
    if selected_plan == PLAN_FREE:
        st.info(
            "Commande créée sous l'option FREE."
            if get_language() == "fr"
            else "Order created under the FREE option."
        )

    elif selected_plan == PLAN_PREMIUM:
        st.info(
            "Commande créée sous l'option PREMIUM. Les frais AfriPay standards s'appliquent à cette commande."
            if get_language() == "fr"
            else "Order created under the PREMIUM option. Standard AfriPay fees apply to this order."
        )
        ...
        st.markdown(
            ...
            <div class="af-warning-card">
                <h4>PREMIUM_PLUS</h4>
                <p>
                    {"Demande enregistrée" if get_language() == "fr" else "Request recorded"} :
                    <strong>{duration_label}</strong><br>
                    {"Montant abonnement" if get_language() == "fr" else "Subscription amount"} :
                    <strong>{premium_plus_price or ('À confirmer' if get_language() == 'fr' else 'To be confirmed')}</strong><br><br>
                    {"Important : votre accès PREMIUM_PLUS et l'avantage 0% frais AfriPay commencent seulement après paiement confirmé et validation administrateur."
                     if get_language() == "fr"
                     else "Important: your PREMIUM_PLUS access and the 0% AfriPay fee benefit start only after confirmed payment and admin validation."}
                </p>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    if selected_plan == PLAN_PREMIUM:
        premium_message = build_upgrade_message(user, PLAN_PREMIUM)
        premium_message += (
            f"\nCode commande : {order_code}\n"
            "Je confirme mon choix PREMIUM et je souhaite recevoir les instructions de paiement."
            if get_language() == "fr"
            else f"\nOrder code: {order_code}\nI confirm my PREMIUM choice and I would like to receive the payment instructions."
        )
        subscription_message = premium_message
        subscription_url = build_whatsapp_url(whatsapp_number, subscription_message)

    elif selected_plan == PLAN_PREMIUM_PLUS:
        premium_plus_message = build_upgrade_message(user, PLAN_PREMIUM_PLUS, selected_duration)
        premium_plus_message += (
            f"\nCode commande : {order_code}\n"
            "Je confirme mon choix PREMIUM_PLUS. Merci de m'envoyer les instructions de paiement de l'abonnement pour activation après validation."
            if get_language() == "fr"
            else f"\nOrder code: {order_code}\nI confirm my PREMIUM_PLUS choice. Please send me the subscription payment instructions for activation after validation."
        )
        subscription_message = premium_plus_message
        subscription_url = build_whatsapp_url(whatsapp_number, subscription_message)

    cart_url = build_whatsapp_url(whatsapp_number, cart_message)
    proof_url = build_whatsapp_url(whatsapp_number, proof_message)

    if selected_plan in {PLAN_PREMIUM, PLAN_PREMIUM_PLUS} and subscription_url:
        button_label = (
            "Confirmer mon choix et recevoir les instructions"
            if get_language() == "fr"
            else "Confirm my choice and receive instructions"
        )
        st.link_button(button_label, subscription_url, width=UI_WIDTH_STRETCH)
    ...
    if selected_plan == PLAN_PREMIUM_PLUS:
        st.caption(
            "Étape suivante : confirmez votre choix PREMIUM_PLUS sur WhatsApp, recevez les instructions de paiement de l'abonnement, puis attendez la validation admin pour activer l'avantage 0%."
            if get_language() == "fr"
            else "Next step: confirm your PREMIUM_PLUS choice on WhatsApp, receive the subscription payment instructions, then wait for admin validation to activate the 0% benefit."
        )
    elif selected_plan == PLAN_PREMIUM:
        st.caption(
            "Étape suivante : confirmez votre choix PREMIUM sur WhatsApp puis poursuivez le parcours de validation et de paiement."
            if get_language() == "fr"
            else "Next step: confirm your PREMIUM choice on WhatsApp, then continue the validation and payment flow."
        )
    else:
...
def main() -> None:
    ...
    if user and get_effective_user_plan(user) == PLAN_FREE and get_free_orders_remaining(user) <= 0:
        if upgrade_available:
            st.warning(tr("upgrade_cta_free"))
            show_upgrade = True
        else:
            render_upgrade_unavailable_notice()
            show_upgrade = False

    if show_upgrade or user:
        if upgrade_available:
